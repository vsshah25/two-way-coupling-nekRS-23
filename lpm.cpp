...
int lpm_t::numNonLocalParticlesTransfer()
{
  auto &data = interp->data();
  auto tidHost = getPropHost("tid");

  int numNonLocal = 0;
  for (int i = 0; i < this->numParticles(); ++i) {
    data.proc[i] = (int) tidHost[i];
    numNonLocal += (data.proc[i] != platform->comm.mpiRank);
  }
...
int lpm_t::numNonLocalParticlesBack()
{
  auto &data = interp->data();
  auto tidHost = getPropHost("pid");
...
void lpm_t::migrateTransfer()
{
  ...
  const auto nUnfound = numUnfoundParticles();
  const auto nNonLocal = numNonLocalParticlesTransfer();
...
void lpm_t::migrateBack()
{
  ...
  const auto nUnfound = numUnfoundParticles();
  const auto nNonLocal = numNonLocalParticlesBack();
